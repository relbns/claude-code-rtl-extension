...
- **Fix: a Hebrew paragraph that opens with an English word no longer renders LTR** (#20) - Claude Code's own stylesheet sets `unicode-bidi: plaintext` on every `p/li/h1-h6/blockquote/td/th` inside its markdown root, so each block takes its direction from its first strong character. A sentence such as "React מרנדר את הקומפוננטה מחדש בכל שינוי state" was rendered left-to-right no matter how much Hebrew followed, which is common in technical chat where a sentence opens with an identifier, a version, or an ordered-list letter. CSS alone cannot fix this because CSS cannot count characters, so a small resolver now runs in the webview: it gives each block the direction of the majority of its words, code spans excluded so a path or an identifier cannot decide a sentence, and tags it. Companion CSS acts on the tag and overrides the first-character guess. A block is only ever touched when its own computed `unicode-bidi` is `plaintext`, which is exactly the condition that causes the bug, so the session header, the session list, tool output and the question and permission dialogs keep the direction the extension already gives them. English-only paragraphs are now tagged LTR and left-aligned instead of being right-aligned along with the rest of the chat. A block whose text is entirely code - a table cell holding one identifier, a paragraph that is a single command - is judged by that code rather than skipped, so it aligns left too. A block is judged only once it stops changing, so a streaming reply is tagged when each paragraph completes rather than on every re-render - otherwise a paragraph opening with an English word reads LTR until enough Hebrew arrives and the text visibly jumps from side to side as it is written. Active, Always and Auto modes all get it, LTR Always is untouched, and Always mode - which previously injected no JS at all - now ships the resolver on its own.
